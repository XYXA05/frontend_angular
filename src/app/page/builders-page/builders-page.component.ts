...
export class BuildersPageComponent implements OnInit {
  ...
  public getMethodOwner(){
    this.http.get(`http://127.0.0.1:8000/user/${this.id}/`).subscribe((data:any) =>{
      console.log(data);
  ...
  public getNews() {
    this.http.get<any[]>(`http://127.0.0.1:8000/get_news_id/${this.id}/`).subscribe((data:any) => {
      this.getNewsData = data;
    });
  }

  public getMethod() {
    this.http.get<any[]>(`http://127.0.0.1:8000/get_items/${this.id}/`).subscribe((data:any) => {
      this.jsonDataa = data;
    });
  }
  ...
  public getVideoUrl(id: string): SafeResourceUrl {
    return this.sanitizer.bypassSecurityTrustResourceUrl(`http://127.0.0.1:8000/get_image/1/${id}/`);
  }
