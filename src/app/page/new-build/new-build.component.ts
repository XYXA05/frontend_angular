...
export class NewBuildComponent implements OnInit{
  ...
  public getMethod(): void {
    this.http.get<any[]>('http://127.0.0.1:8000/items/').subscribe(
      data => {
        this.jsonData = data;
  ...
  public getMethods(id: number): void {
    this.http.get(`http://127.0.0.1:8000/items/${id}/`).subscribe((data: any) => {
      console.log(data);
      this.jsonData = data;
...
filterItems(
  ...
  this.http.get<DescriptionItem[]>('http://127.0.0.1:8000/filter_items/', { params }).subscribe(
      (data) => {
